    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Login([Bind] DangNhap dangNhap)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    string role = "";
                    if (nguoiDung.Quyen == quyen.Admin)
                    {
                        role = "Admin";
                    }
                    else if (nguoiDung.Quyen == quyen.QuanLy)
                    {
                        role = "QuanLy";
                    }
                    else
                    ...
                    await HttpContext.SignInAsync(CookieAuthenticationDefaults.AuthenticationScheme,
                                                  new ClaimsPrincipal(claimsIdentity),
                                                  authProperties);
                    return LocalRedirect(dangNhap.LienKetChuyenTrang ?? (nguoiDung.Quyen == quyen.Admin ? "/Admin" : nguoiDung.Quyen == quyen.QuanLy ? "/QuanLy" : "/"));
                }
            }
            return View(dangNhap);
        }
